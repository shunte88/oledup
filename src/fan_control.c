...
bool init_serial(const char *serial_name) {
    ...
    if (serial_port < 0) {
        printf("Cannot open /dev/ttyUSB0 serial port ErrorCode: %s\n",
               strerror(errno));
        printf("Please check the /boot/config.txt file and add dtoverlay=dwc2, "
               "dr_mode=host and reboot your Pi\n");
        ret = false;
    }
    if (ret) {
...
int __init_serial() { return close(serial_port); }
...
bool fanControl(double temperature, bool *goser) {
    ...
    if (!goser)
        goser = (bool *)init_serial("/dev/ttyUSB0");

    if (temperature < cpu_temp_conf[0]) {
        sprintf(data, "pwm_%03d", 0);
        ret = false;
    } else if (temperature >= cpu_temp_conf[0] &&
               temperature < cpu_temp_conf[2]) {
        sprintf(data, "pwm_%03d", cpu_temp_conf[1]);
    } else if (temperature >= cpu_temp_conf[2] &&
               temperature < cpu_temp_conf[4]) {
        sprintf(data, "pwm_%03d", cpu_temp_conf[3]);
    } else if (temperature >= cpu_temp_conf[4] &&
               temperature < cpu_temp_conf[6]) {
    ...
    if (goser)
        ret = (0 != send_serial((const char *)&data));
