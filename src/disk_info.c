...
int get_short_devfs_name(char *devicename) {
    ...
    if ((ptr = strchr(devicename, '/'))) {
        ...
        while ((ptr = strchr(ptr, '/'))) {
            ...
            while (tolower(ptr[0]) >= 'a' && tolower(ptr[0]) <= 'z')
                ptr++;
...
void get_disk_stats_proc(char verbose) {
    ...
    bool diskstats = true;
    static char proc_stat[MAX_LINE_BUFFER] = {0};

    int hidden_if = 0, current_if_num = 0;
    iface_speed_stats_t stats; /* local struct, used to calc total values */
    iface_speed_stats_t tmp_if_stats;

    memset(&stats, 0, (size_t)sizeof(iface_speed_stats_t)); /* init it */
    /* dont open proc_net_dev if netstat_i is requested
    else try to open and if it fails fallback */
    if (diskstats && !(f = fopen(DISKSTATS_FILE, "r"))) {
        diskstats = false;
    }
    ...
    while ((fgets(buffer, MAX_LINE_BUFFER, f) != NULL)) {
        n = sscanf(buffer,
                   (diskstats ? "%i %i %s %llu%llu%llu%llu%llu%llu%llu%*i"
                              : "%i %i %*i %s %llu%llu%llu%llu%llu%llu%llu%*i"),
                   &major, &minor, name, &tmp_if_stats.packets.in,
                   &tmp_if_stats.errors.in, &tmp_if_stats.bytes.in, &tmp_long,
                   &tmp_if_stats.packets.out, &tmp_if_stats.errors.out,
            ...
            if (n != 10) {
                if (!diskstats && n == 3) {
                    ...
                    if (proc_stat[0] == 0) {
                        ...
                        strcpy(proc_stat, PARTITIONS_FILE);
                        if ((ptr = strrchr(proc_stat, '/'))) {
                            ...
                            free(name);
                            free(buffer);
                            deinit(1, "strange /proc/partitions name, couldnt "
                                      "build /proc/stats name\n");
                        }
                    }
                    ...
                    while ((fgets(buffer, MAX_LINE_BUFFER, f_s) != NULL)) {
                        if (!strncmp("disk_io:", buffer, 8)) {
                            ...
                            while (ptr[0] != 0) {
                                n = sscanf(
                                    ptr, "(%i,%i): (%*i,%llu,%llu,%llu,%llu)",
                                    &maj_s, &min_s, &tmp_if_stats.packets.in,
                                    &tmp_if_stats.bytes.in,
                                    &tmp_if_stats.packets.out,
                                    &tmp_if_stats.bytes.out);
        ...
        hidden_if = process_if_data(hidden_if, tmp_if_stats, &stats, name,
                                    current_if_num, verbose,
                                    (n == 10 || proc_stat[0] != 0));
        current_if_num++;
    } /* fgets done (while) */
