...
void drawIP(DrawAttr *disp) {
    ...
    for (size_t i = 0; i < strlen(disp->value); i++) {
        ...
        switch (cc) {
            case '.': start = 11 * el; wa = 6; break;
            case ':': start = 12 * el; wa = 6; break;
            case ' ': start = 13 * el; wa = 6; break;
            case '?': start = 14 * el; break;
            default: start = (cc - 48) * el;
        }
        memcpy(dest, thinman24x8() + start, sizeof dest);
        ssd1306_drawBitmap(x-((6==wa)?2:0), disp->pos.y, dest, szw, szh, WHITE);
        x+=wa;
    }
}
    ...
    for (size_t i = 0; i < strlen(disp->value); i++) {
        ...
        if ((i > ll) || (disp->value[i] != disp->lastval[i])) {
            if ((i > ll) || ('X' == disp->lastval[i])) {
                w = ((disp->lastval[i]==':')||(disp->lastval[i]==' '))?sepWidth:dt->charWidth;
                ssd1306_fillRect((w==sepWidth)?x-offset:x, dt->pos.y - 1, w,
                                 dt->charHeight + 2, BLACK);
            } else {
                w = ((disp->lastval[i]==':')||(disp->lastval[i]==' '))?sepWidth:dt->charWidth;
                ssd1306_drawChar((w==sepWidth)?x-offset:x, dt->pos.y, disp->lastval[i], BLACK, tsize); // soft erase
            }
            w = ((disp->value[i]==':')||(disp->value[i]==' '))?sepWidth:dt->charWidth;
            ssd1306_drawChar((w==sepWidth)?x-offset:x, dt->pos.y, disp->value[i], WHITE, tsize);
        }
        w = ((disp->value[i]==':')||(disp->value[i]==' '))?sepWidth:dt->charWidth;
        x += w;
    }
...
int main(void) {
    ...
    while (true) {
        ...
        while(--trick) {
            ...
            if (sysinfo(&sys_info) != 0) {
                ...
                for(int initr=0; initr<4; initr++)
                    if (0==attrs[initr].lastval[0]) continue;
                    else attrs[initr].lastval[0] = 0;
                ...
                int z = readableUnits(usedBuff, mem_info.usedMem, sizeof(usedBuff));
                z = readableUnits(totalBuff, mem_info.totalMem, sizeof(totalBuff));
                sprintf(attrs[AT_MEM_INFO].value, " %s/%s", usedBuff, totalBuff);
                ...
                while (ifAddrStruct != NULL) {
                    if (ifAddrStruct->ifa_addr != NULL && ifAddrStruct->ifa_addr->sa_family == AF_INET) {
                        ...
                        if (strcmp(ifAddrStruct->ifa_name, "eth0") == 0) {
                            sprintf(attrs[AT_NET_INFO].value, "%s", addressBuffer);
                ...
                sprintf(attrs[AT_DISK_INFO].value, " %s/%s", usedBuff, totalBuff);

                // less of a helper with the idx based impl. clean enough so a keeper
                #define dispAttr(disp)								\
                            if (strcmp(disp.value,disp.lastval)!=0) {				\
                                if(strcmp(disp.lastval,INIT_ATTR_VALUE)!=0)paint = true;	\
                                if(disp.forceClear){						\
                                    ssd1306_clearDisplay();					\
                                    paint=true;							\
                                }								\
                                if(IT_NULL_ICON!=disp.icon)putIcon(&disp);			\
                ...
                if(attrs[frame].visible) {
                    if (frame==AT_NET_INFO) {
                        drawIP(&attrs[frame]); // display IP - specialized tall-skinny font
                    } else {
        ...
        while(--trick){
            ...
            if(loctm->tm_sec % 2) attrs[AT_CLOCK_INFO].value[2] = 32;
