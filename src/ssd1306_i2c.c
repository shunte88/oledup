...
void ssd1306_stopscroll(void) { ssd1306_command(SSD1306_DEACTIVATE_SCROLL); }
...
void ssd1306_clearDisplay(void) {
    ...
    ssd1306_drawRect(0, 0, SSD1306_LCDWIDTH, SSD1306_LCDHEIGHT, BLACK);
    cursor_y = 0;
    cursor_x = 0;
}

void ssd1306_drawFastHLineInternal(int x, int y, int w, uint16_t color) {
...
void ssd1306_drawFastVLineInternal(int x, int __y, int __h, uint16_t color) {
    ...
    if (mod) {
        ...
        static uint16_t premask[8] = {0x00, 0x80, 0xC0, 0xE0,
                                      0xF0, 0xF8, 0xFC, 0xFE};
    ...
    if (h) {
        ...
        // note - lookup table results in a nearly 10% performance
        // improvement in fill* functions
        static uint16_t postmask[8] = {0x00, 0x01, 0x03, 0x07,
                                       0x0F, 0x1F, 0x3F, 0x7F};
...
void ssd1306_drawRect(int x, int y, int w, int h, uint16_t drawcolor) {
    ...
    ssd1306_drawFastHLine(x, y, w, drawcolor);
    ssd1306_drawFastVLine(x, y, h, drawcolor);
    ssd1306_drawFastHLine(x, y + h - 1, w, drawcolor);
    ssd1306_drawFastVLine(x + w - 1, y, h, drawcolor);
}
...
void ssd1306_displayOn(void) { ssd1306_command(SSD1306_DISPLAYON); }

void ssd1306_displayOff(void) { ssd1306_command(SSD1306_DISPLAYOFF); }

int textsize = 1;
int wrap = 1;

void ssd1306_setTextSize(int s) { textsize = (s > 0) ? s : 1; }
